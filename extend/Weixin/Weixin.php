...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    public function checkAuth($refresh = 0, $path = self::TOKEN_PATH)
    {
        ...
        if ($data->expire_time < time() || $refresh) {
            ...
            if ($access_token) {
                ...
                $data->date_time = date('Y-m-d H:i:s', $data->expire_time);
     ...
     */
    public function getJsSign($param)
    {
        $url = 'http://' . $_SERVER[HTTP_HOST] . $_SERVER[REQUEST_URI] . $param;
        //echo $url;
        $this->jsapi_ticket = $this->getJsTicket($this->appid);
    ...
    public function getCardApi()
    {
        $filepath = "cardapi_token.json";//待读取文件的相对路径
        $targetfile = realpath($filepath);
